...
export default {
  ...
  created () {
    ...
    switch (routetype) {
      case 'collect_topics':
        lists = this.user.userinfo.collect_topics;
        count = lists.length;
        break;
      case 'recent_replies':
        lists = this.user.userinfo.recent_replies;
        count = this.REPLIES_COUNT;
        break;
      case 'recent_topics':
        lists = this.user.userinfo.recent_topics;
        count = this.MY_TOPICS_COUNT;
        break;
      case 'post_topics':
        lists = this.user.userinfo.post_topics;
        count = this.MY_TOPICS_COUNT;
        break;
    };
  ...
  methods: {
    ...
    tapToInfo (topicid, userid) {
      ...
      // 点击打开详情页时匹配该文章是否被收藏      
      //this.checkCollected(this.user.userinfo.collect_topics, topicid)  
    },
    // 检查该文章是否被收藏
    // ================
